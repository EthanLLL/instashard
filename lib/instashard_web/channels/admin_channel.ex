defmodule InstashardWeb.AdminChannel do
  ...
  alias Instashard.Backend.{DbRegistry, Migration, MigrationGate, Pool, ShardMapping}
  ...
  def handle_in("migration:start", %{"shard" => shard, "target_db" => target_db}, socket) do
    case Migration.start_migration(shard, target_db) do
      :ok ->
        {:noreply, socket}
      {:error, reason} ->
  ...
  def handle_in("migration:cutover", %{"shard" => shard}, socket) do
    case Migration.cutover(shard) do
      :ok -> {:noreply, socket}
      {:error, reason} ->
        broadcast_migration_event(shard, "error", inspect(reason))
        {:noreply, socket}
  ...
  def handle_in("migration:cancel", %{"shard" => shard}, socket) do
    case Migration.cancel(shard) do
      :ok -> {:noreply, socket}
      {:error, reason} ->
        broadcast_migration_event(shard, "error", inspect(reason))
        {:noreply, socket}
  ...
  def handle_info(:push_migration_status, socket) do
    ...
    push(socket, "migration:status", %{status: status})
    {:noreply, socket}
  end
  ...
  # ── Public broadcast helpers (called by Migration process) ─────────────
